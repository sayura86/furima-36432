  ...
  describe '会員情報入力' do
    ...
    context '新規登録できない場合' do
      ...
      it "フリガナ（名字）が空だと登録できない" do
        ...
        @user.valid?
        expect(@user.errors.full_messages).to include("First name kana can't be blank", "First name kana First name kana Full-width characters")
      end
      it "フリガナ（名前）が空だと登録できない" do
        @user.last_name_kana = ''
        @user.valid?
        expect(@user.errors.full_messages).to include("Last name kana can't be blank", "Last name kana Last name kana Full-width characters")
      end
